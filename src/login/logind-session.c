...
int session_set_leader(Session *s, PidRef leader) {
        int r;

        assert(s);
        assert_se(pidref_is_set(&leader));

        if (s->leader.pid == leader.pid)
                return 0;
...
int session_load(Session *s) {
        ...
        if (leader) {
                ...
                if (r < 0)
                        ...
                        r = pidref_set_pid(&p, pid);
                        if (r < 0)
                                return r;

                        r = session_set_leader(s, TAKE_PIDREF(p));
                        if (r < 0)
