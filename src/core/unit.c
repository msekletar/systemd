...
FreezerState unit_freezer_state_kernel(Unit *u) {
        char *values[1] = {};
        int r;
...
static int unit_freezer_action(Unit *u, FreezerAction action) {
        UnitActiveState s;
        FreezerState target, transition;
        int (*method)(Unit*);
        int r;
        ...
        if (action == FREEZER_FREEZE) {
                target = FREEZER_FROZEN;
                transition = FREEZER_FREEZING;
                method = UNIT_VTABLE(u)->freeze;
        } else {
                target = FREEZER_RUNNING;
                transition = FREEZER_THAWING;
                method = UNIT_VTABLE(u)->thaw;
        }

        if (!method || cg_all_unified() <= 0 || access("/sys/fs/cgroup/init.scope/cgroup.freeze", F_OK) < 0)
                return -EOPNOTSUPP;

        if (u->job || u->load_state != UNIT_LOADED)
                return -EBUSY;

        s = unit_active_state(u);
        if (!IN_SET(s, UNIT_ACTIVE))
                return -EBUSY;

        if (IN_SET(u->freezer_state, FREEZER_FREEZING, FREEZER_THAWING))
                return -EALREADY;

        if (unit_freezer_state_kernel(u) == target) {
                u->freezer_state = target;
                return 0;
        }

        u->freezer_state = transition;
