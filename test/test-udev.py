...
def net_id_run(netdev: Path, scheme='v262', rules='') -> dict[str, str]:
...
@pytest.mark.parametrize('scheme', ['v261', 'v262'])
...
def test_net_id_vf_ports(net_id_setup, scheme, port):
    ...
    for vfnum, vf in enumerate(vfs[port]):
        ...
        net_id_assert_names(properties, port, f'np{port}v{vfnum}' if scheme == 'v262' else f'v{vfnum}')
...
def test_net_id_vf_port_precedence(net_id_setup, scheme, vf_port_name):
    ...
    port_suffix = 'nvfport' if vf_port_name == 'vfport' else 'np0' if scheme == 'v262' else 'd7'
